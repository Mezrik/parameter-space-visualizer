...
import { createVariableTokens } from '../src/helpers/scatter';
import { Token, VariableInterval } from '../src/types/expression';
import { calculateSampling } from '../src/lib/data/utils';
import { CustomScatterPlot } from '../src/ScatterPlot';

const paramVis = require('../umd');

import 'jsdom-worker';

benchmarkSuite('Expression sampling', {
  setupSuite() {
    simple3DExpressionTest.subs = createParameterInstantiation(
      150,
      simple3DExpressionTest.intervals,
    );

    simple3DExpressionTest.tokens = simple3DExpressionTest.intervals.map(({ name }) =>
      createVariableTokens(name),
    );

    simpleExpression2DTest.subs = createParameterInstantiation(
      80,
      simpleExpression2DTest.intervals,
    );

    simpleExpression2DTest.tokens = simpleExpression2DTest.intervals.map(({ name }) =>
      createVariableTokens(name),
    );
    complex2DExpressionTest.subs = createParameterInstantiation(
      80,
      complex2DExpressionTest.intervals,
    );

    complex2DExpressionTest.tokens = complex2DExpressionTest.intervals.map(({ name }) =>
      createVariableTokens(name),
    );
  },
  ['Bench simple3DExpressionTest']: () => {
    const { subs, exp, tokens } = simple3DExpressionTest;
    calculateSampling(subs, exp, tokens);
  },
  ['Bench simpleExpression2DTest']: () => {
    const { subs, exp, tokens } = simpleExpression2DTest;
    calculateSampling(subs, exp, tokens);
  },
  ['Bench complex2DExpressionTest']: () => {
    const { subs, exp, tokens } = complex2DExpressionTest;
    calculateSampling(subs, exp, tokens);
  },
...
const createParameterInstantiation = (density: number, intervals: VariableInterval[]) => {
  if (!intervals.length) throw Error('At least one interval has to be defined.');

  const list = new Array(density * density).fill(0);
  const distDom = [0, density - 1];

  const scales = intervals.reduce<Record<string, ScaleLinear<number, number>>>(
    (acc, interval) => ({
      ...acc,
      [interval.name]: scaleLinear().domain(distDom).range([interval.start, interval.end]),
    }),
    {},
  );

  const [x, y, ...rest] = Object.keys(scales);

  return list.map((_, i) => {
    return {
      [x]: scales[x](Math.floor(i / density)),
      [y]: scales[y](i % density),
      ...rest.reduce<Record<string, number>>(
        (acc, name) => ({ ...acc, [name]: scales[name](0) }),
        {},
      ),
    };
  });
