...
const createProbabilityChart = (expression: string, container: HTMLElement) => {
  container.classList.add("probability-sampling");
  ...
  `,
    "probability-sampling-chart-styles"
  );
...
document.addEventListener("DOMContentLoaded", (e) => {
  ...
  const probabilitySamplingChart = createProbabilityChart(
    "(-1 * ((p+(-1)) * (q*r+(-1)*r+(-1)*q+1)))/(q*r+(-1)*r+1)",
